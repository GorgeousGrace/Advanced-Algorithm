...
def save_dataset(n: int, filename="login_dataset.csv"):
    """Generates and saves a dataset of login names."""
    logins = generate_logins(n)
    df = pd.DataFrame(logins, columns=["Login_ID"])
    df.to_csv(filename, index=False)
    print(f"Dataset saved to {filename}")


def generate_username():
    """Generate a random username consisting of a name and random numbers."""
    names = ["alice", "bob", "charlie", "david", "eva", "frank", "grace", "henry", "isabel", "jackson"]
    suffix = ''.join(random.choices(string.digits, k=4))
    return random.choice(names) + suffix

def save_username_dataset(n: int, filename="login_usernames.csv"):
    """Generates and saves a dataset of login usernames."""
    usernames = [generate_username() for _ in range(n)]
...
# Example Usage
save_username_dataset(10_000_000)  # Save dataset with 10 million usernames
